import Coders from "../models/devit.models.js";
export const postDevit = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ error: false, msg: "Coders posted successfully", devit });
  } catch (error) {
    console.log(error);
...
export const getDevits = async (req, res) => {
  try {
    const devits = await Coders.find().sort({ _id: -1 });
    res
      .status(200)
...
export const getDevit = async (req, res) => {
  try {
    const devit = await Coders.findById(req.params.id);
    if (!devit)
      return res.status(404).json({ error: true, msg: "Coders not found" });
    res
      .status(200)
      .json({ error: false, msg: "Coders fetched successfully", devit });
  } catch (error) {
    res.status(500).json({ error: true, msg: "Internal Server Error" });
  }
};
export const updateDevit = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ error: false, msg: "Coders updated successfully", devit });
  } catch (error) {
    res.status(500).json({ error: true, msg: "Internal Server Error" });
  }
};
export const deleteDevit = async (req, res) => {
  try {
    ...
    if (!devit)
      return res.status(404).json({ error: true, msg: "Coders not found" });
    await devit.delete();
    res.status(200).json({ error: false, msg: "Coders deleted successfully" });
  } catch (error) {
    res.status(500).json({ error: true, msg: "Internal Server Error" });
  }
};
export const getMyDevits = async (req, res) => {
  ...
  const devits = await Coders.find({ userid: user_id });
  //Now fetch the redevits of the user by userid
  const redevits = await Coders.find({
...
export const likeDevit = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(200)
        .json({ error: false, msg: "Coders unliked successfully", count });
    }
    ...
    res
      .status(201)
      .json({ error: false, msg: "Coders liked successfully", count });
  } catch (error) {
    console.log(error);
    res.status(500).json({ error: true, msg: "Internal Server Error" });
  }
...
export const redevitDevit = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(200)
        .json({ error: true, msg: "Coders unredevited successfully", count });
    }
    ...
    res
      .status(201)
      .json({ error: false, msg: "Coders redevited successfully", count });
  } catch (error) {
    res.status(500).json({ error: true, msg: "Internal Server Error" });
  }
};
...
export const getTrends = async (req, res) => {
  try {
    const devits = await Coders.find();
    const trends = [];
